...
## Rewrite text from an online LLM

Text produced by an online LLM can be regenerated by the local model with the
statistical watermark:

1. Put your own formula or specification in `private/watermark.txt`. Do not use
   a third party's formula without authorization.
2. Generate the identity and keys as described above.
3. Evaluate your function for one complete period and save exactly
   `WATERMARK_PERIOD` numbers in `private/intensity.json`.
4. Save the online LLM output in `private/original-text.txt`.

Example table structure for period 4—the values below are illustrative and
must be replaced with values from your function:
...
```bash
run rewrite-local \
  --input private/original-text.txt \
  --output private/watermarked-text.txt \
  --device cpu \
  --max-new-tokens 128
```

The command uses the same `.env` settings as `session-local`. It instructs the
local model to preserve meaning, facts, names, numbers, citations, code,
commands, URLs, hashes, identifiers, equations, and structured data while
applying the bias before each new token.

This process creates new text; it does not invisibly alter the same bytes.
Models can introduce differences, so review the result before publication.
`APPLIED` confirms logits modulation, not perfect semantic equivalence.
